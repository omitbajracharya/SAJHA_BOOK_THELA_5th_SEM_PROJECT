...
<!-- /////starting of col//////////////// -->
  ...
  <img class="rounded-circle  mt-2" src="images/cart.jpg" alt="Card image" style="width:80%;height:50%;margin-left:10%;">
    ...
    <button class="btn btn-primary">
      	...
      	<?php
      	 require_once("DBConnect.php");
      	 $data = mysqli_query($conn,"SELECT COUNT(`pid`) AS num FROM `seller_cart` WHERE `username` = '$u'") or die(mysql_error());
  ...
  <img class="rounded-circle mt-2" src="images/sell.jpg" alt="Card image" style="width:80%;height:50%;margin-left:10%;">
    <div class="card-body">
      ...
      <button class="btn btn-primary">
      	...
      	<?php
      	 require_once("DBConnect.php");
      	 $data = mysqli_query($conn,"SELECT COUNT(`id`) AS num FROM `seller_transaction` WHERE `username` = '$u'") or die(mysql_error());
         $row = mysqli_fetch_assoc($data);
         $total = $row['num'];
         echo $total;
