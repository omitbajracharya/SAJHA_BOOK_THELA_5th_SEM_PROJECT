...
<link rel="stylesheet" type="text/css" href="rating_style.css">
  <script type="text/javascript">
   function change(id)
   {
     ...
     // alert(cname);
      var ab=document.getElementById("hidden_"+id).value;
      // alert(ab);
      // document.getElementById("rating"+cname).innerHTML=ab;

      for(var i=ab;i>=1;i--)
      {
         document.getElementById(i+cname).src="star2.png";
      }
      var bc=parseInt(ab)+1;
      for(var j=bc;j<=5;j++)
      {
         document.getElementById(j+cname).src="star1.png";
      }
       window.location.href = "insertrating.php?id=" + cname + "&rate=" + ab;
   }
</script>
<?php
...
?>
  ...
  <div class="div">
    ...
    <img src="<?php  echo $row['star1']; ?>" onclick="change(this.id);" id="1<?php echo $productid?>" class="<?php echo $productid?>">
    <input type="hidden" id="hidden_2<?php echo $productid?>" value="2">
    <img src="<?php  echo $row['star2']; ?>" onclick="change(this.id);" id="2<?php echo $productid?>" class="<?php echo $productid?>">
    <input type="hidden" id="hidden_3<?php echo $productid?>" value="3">
    <img src="<?php  echo $row['star3']; ?>" onclick="change(this.id);" id="3<?php echo $productid?>" class="<?php echo $productid?>">
    <input type="hidden" id="hidden_4<?php echo $productid?>" value="4">
    <img src="<?php  echo $row['star4']; ?>" onclick="change(this.id);" id="4<?php echo $productid?>" class="<?php echo $productid?>">
    <input type="hidden" id="hidden_5<?php echo $productid?>" value="5">
    <img src="<?php  echo $row['star5']; ?>" onclick="change(this.id);" id="5<?php echo $productid?>" class="<?php echo $productid?>"><br>
